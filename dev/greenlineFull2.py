...
def calculate_slope_and_annotate(ax, midpoints_x, midpoints_y, person_idx):
...
for person_idx, person in enumerate(selected_instances):
    ...
    if midpoints_x_img.size != 0 and midpoints_y_img.size != 0: 
        calculate_slope_and_annotate(ax2, midpoints_x_img, midpoints_y_img, person_idx)
    
    # Plot a line connecting the midpoints
    ax2.plot(midpoints_x_img, midpoints_y_img, 'g-')
